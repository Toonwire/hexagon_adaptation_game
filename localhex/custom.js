...
function Hexagon(x, y, z, layout) {
	...
	if (layout.orientation == Orientation.FLAT) {
		hex_center.x = layout.center.x + 3/2 * layout.hex_radius * -z;
		hex_center.y = layout.center.y + 3**0.5 * layout.hex_radius * (-z/2 + -y);

	} else if (layout.orientation == Orientation.POINTY) {
		hex_center.x = layout.center.x + 3**0.5 * layout.hex_radius * (-z/2 + -y);
		hex_center.y = layout.center.y + 3/2 * layout.hex_radius * -z;

	} else throw "Invalid orientation: Must be either FLAT or POINTY, was " + layout.orientation;

	var corners = [];
	for (var i = 0; i < 6; i++) {
		corners.push(hex_corner(layout.orientation, hex_center, layout.hex_radius, i));
	}

	return {
				cube:{x:x, y:y, z:z}, center: hex_center, corners:corners, radius:layout.hex_radius, 
...
function makeHexagons(layout) {

	// how many hexagon rings can fit in the width/height?
	var width_increase_per_ring = -1, height_increase_per_ring = -1;

	if (layout.orientation == Orientation.FLAT) {
		// initial center hexagon dimensions
		var hex_width = 2 * layout.hex_radius;
		var hex_height = 3**0.5 * layout.hex_radius;
		var hex_center_spacing_horizontal = hex_width * 3/4;
		var hex_center_spacing_vertical = hex_height;

		// one ring adds one more hexagons in both directions
		//width_increase_per_ring = 2*hex_center_spacing_horizontal + 2*hex_radius; 
		width_increase_per_ring = 2*hex_center_spacing_horizontal;
		height_increase_per_ring = 2*hex_height;

	}

	else if (layout.orientation == Orientation.POINTY) {
		// initial center hexagon dimensions
		var hex_width = 3**0.5 * layout.hex_radius;
		var hex_height = 2 * layout.hex_radius;
		var hex_center_spacing_horizontal = hex_width;
		var hex_center_spacing_vertical = hex_height * 3/4;

		// one ring adds one more hexagons in both directions
		width_increase_per_ring = 2*hex_width;
		height_increase_per_ring = 2*hex_center_spacing_vertical;
		//height_increase_per_ring = 2*hex_center_spacing_vertical + 2*hex_radius;
	}

	// this ring amount also count the center hexagon, not to confuse with grid radius (actual "rings" around center)
	var numRings = Math.min(Math.floor(layout.width / width_increase_per_ring), Math.floor(layout.height / height_increase_per_ring));
	var cubeCoords = getCubeCoords(numRings);

	return hexagonsFromCubeCoords(cubeCoords, layout);
}

function readFileAsString(evt) {
  	...
  	for (var i = 0, f; f = files[i]; i++) {
	    ...
	    reader.onload = (function(file) {
	    	...
	    	return function(event) {
			    ...
			    var fBody = getFunctionBody(content);
			    var turnCode = compileCode(fBody);
	    		var algoName = file.name.substring(0, file.name.lastIndexOf("."));
			    algos.push({name:algoName, code:turnCode});

				if (algos.length > 1) // cannot start a round without atleast two players/algos
...
function setupGame(ori=Orientation.POINTY) {
	var layout = Layout(canvas.width, canvas.height, ori, hex_radius);

	if (algos.length < 2) console.log("Two players required to start"); // just in case
...
// TODO: also count up playerExceptions if not valid transaction?
...
function startGame() {
	...
	gameTimer = setInterval(start, 100);
}
...
function onResize() {
	// TODO: remake hexagons with current state, but different dimens
}
...
function getFunctionBody(turnFunction) {
 	return turnFunction.substring(turnFunction.indexOf("{") + 1, turnFunction.lastIndexOf("}"));
}
...
function drawHexagons(hexagons) {
	...
	for (var i = 0; i < hexagons.length; i++) {
		...
		context.fillText(hexagon.resources, hexagon.center.x, hexagon.center.y+hexagon.radius/4, hexagon.radius);	
...
function getBoardState(player) {
	// playerCell 	-> id, resources, owner, maxGrowth, neighborCells
...
function getGridRadius(hexagons) {
...
function setupHexagonNeighbors(hexagons, wraparound) {
	...
	var numRings = getGridRadius(hexagons);
	...
	for (var i = 0; i < hexagons.length; i++) {
		...
		for (var c = 0; c < cubeNeighbors.length; c++) {
			...
			// OPTION 2 - not adding both way, but keep neighbor indices the same
...
function makeBoard(players, layout) {
	...
	var minDimen = Math.min(canvas.width, canvas.height);
	...
	var cubeCoords = getCubeCoords(numRings);
	var hexagons = hexagonsFromCubeCoords(cubeCoords, layout);
